...
脚本会创建独立的 `.venv-build-windows` 构建环境，产物为 `release\windows\ScreenshotStitcher-v1.2.0-Windows-x64.exe`。exe 中包含 Python、OpenCV、NumPy 和 Pillow，最终用户无需安装 Python。

打包完成后，可先在 Windows 10/11 上双击运行一次进行验证，然后将这一个带版本号的 EXE 上传到对应的 GitHub Release。
